...
`Authenticator` turns an inbound request into a `*Principal` (or `nil`
...
`request` is intentionally `any` — `core` doesn't know or care whether
the adapter hands it a `*fiber.Ctx` or anything else; only the adapter
(and your own `Authenticator` implementation) needs to agree on that.
...
- `core.NewAllowAllAuthenticator(principal)` — authenticates every
  request as the same fixed `Principal`.
- `core.DenyAllAuthenticator{}` — authenticates nobody, useful for
  asserting a login gate actually blocks access.

## The login page

The framework ships the login *page* — the form, the failure message,
the CSRF check, the redirect back to wherever the visitor was headed. It
...
minting a session token is what keeps it out of key management: it has
no signing secret, and needs none.

The two halves therefore split like this:

| | reads a session | creates one |
|---|---|---|
| interface | `Authenticator` | `LoginBackend` |
| method | `Authenticate(request)` | `VerifyCredentials` / `BeginSession` / `EndSession` |

```go
sessions := NewCookieSessionBackend()      // your implementation
admin := core.New(
...
One type usually implements both, since they have to agree on the
session format. A complete, runnable implementation — PBKDF2 password
hashing, an HMAC-signed cookie — is in `examples/fiber/session.go`; copy
it and replace the in-memory user table.

**Registering a `LoginBackend` is the switch.** With one:

- `{basePath}/login` and `{basePath}/logout` are mounted. Both are
...
The `?next=` destination is validated (`core.SafeNextURL`) to be a path
...
off-site, scheme-relative, or outside `basePath` falls back to the
admin's home.
...
If `core.New` isn't given an `Authenticator` at all, every request is
treated as authenticated (with a `nil` principal) — this isn't a secure
...
If `Authenticator.Authenticate` returns `nil`, the adapter responds
