...
`core.ResourcePermission(slug, action)` builds `"{slug}.{action}"` — `"users.view"`, `"users.delete"`,
...
`core.WithPagePermission(...)` at registration to use a
different string.
...
(`computePermissions`) also flow into the
...
- **A cascade into a type the principal may not delete blocks the whole
  ...
  ModelAdmin's own `CanDelete`, then the Authorizer. A type the admin
...
- `core.AllowAllAuthorizer{}` — grants every permission
  unconditionally.
- `core.DenyAllAuthorizer{}` — denies every permission; useful for
  asserting a gate actually blocks something.
- `core.SuperuserAuthorizer{}` — grants every permission to a
  `Principal` with `IsSuperuser` set, denies everyone else. Note that
  this is genuinely all-or-nothing: a signed-in non-superuser is
  refused *every* permission, `dashboard.view` included, and meets a
  bare "Permission denied." on every page. For anything with more than
  one kind of user, write an `Authorizer` that distinguishes reads from
  writes — `examples/fiber/session.go`'s `ReadOnlyForNonSuperusers` is
  the smallest example.

## No authorizer configured

Same as with no `Authenticator`: every permission is granted by
default if `core.New` has no `Authorizer` set at all. Set one before anything resembling production traffic.
