...
EXAMPLE_DIR = os.path.join(REPO_ROOT, "examples", "fiber")
...
def app_server():
    ...
    # start_new_session: `go run` compiles and then execs the binary as a
    # child, so signalling go alone leaves that child holding the port.
    process = subprocess.Popen(
        ["go", "run", "."],
        cwd=EXAMPLE_DIR,
        env={**os.environ, "ADMIN_SESSION_SECRET": SESSION_SECRET, "PORT": str(PORT)},
