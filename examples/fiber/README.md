...
Reference Fiber application exercising go-polyadmin: `User`,
...
The admin is behind a real login (`session.go`), not a hardcoded
...
edit/delete controls are simply absent — `computePermissions` asks the
...
The rule lives in `ReadOnlyForNonSuperusers` (`session.go`), not
`core.SuperuserAuthorizer`: that one is all-or-nothing, so a
non-superuser would be refused *every* permission including
`dashboard.view` and would meet a bare "Permission denied." on every
page.
...
```bash
ADMIN_SESSION_SECRET=$(openssl rand -hex 32) go run .
```

`session.go` is meant to be copied and adapted — swap the in-memory
account table for your own user store. See
[`../../docs/authentication.md`](../../docs/authentication.md#the-login-page).

See [`../../docs/`](../../docs/) for the reference documentation.
